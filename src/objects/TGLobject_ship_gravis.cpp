...
bool TGLobject_ship_gravis::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (k->m_button[0] && !k->m_old_button[0] && m_fuel>40) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		bullet=new TGLobject_bullet(float(get_x()+(cos_degree(a)*12)),float(get_y()+(sin_degree(a)*12)),0,m_angle,6,1,GLTM->get("graphics/objects/bullet-darkblue.png"),this);
	...
	if (m_attracting) {
		if ((m_attracting_cycle%25)==0) {
			map->add_auxiliary_back_object(new TGLobject_FX_particle(get_x(),get_y(),m_angle,0,0,0,false,0.5f,0,0.5f,2.0f,50,GLTM->get("graphics/objects/ship-gravis-1.png")));
		} // if 
	} // if 
	...
	if (m_thrusting) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		if ((m_cycle%4)==0) map->add_auxiliary_back_object(new TGLobject_FX_particle(float(get_x()+(cos_degree(a)*8)-(sin_degree(a)*8)),float(get_y()+(sin_degree(a)*8)+(cos_degree(a)*8)),rand()%60,0,0,1,false,0.125f,0,0.25f,0.75f,50,GLTM->get("graphics/objects/smoke.png")));
		if ((m_cycle%4)==0) map->add_auxiliary_back_object(new TGLobject_FX_particle(float(get_x()-(cos_degree(a)*8)-(sin_degree(a)*8)),float(get_y()-(sin_degree(a)*8)+(cos_degree(a)*8)),rand()%60,0,0,1,false,0.125f,0,0.25f,0.75f,50,GLTM->get("graphics/objects/smoke.png")));
	...
	if (m_attracting) {
		if (m_attracting_channel==-1) {
			m_attracting_channel=Sound_play_continuous(SFXM->get(TGL_SFX_FOLDER "/gravis"),sfx_volume);
		} // if 
	} else {
		if (m_attracting_channel!=-1) {
			Mix_HaltChannel(m_attracting_channel);
...
void TGLobject_ship_gravis::draw(glm::mat4 transform, class GLTManager *GLTM)
{
	if (m_last_mask==0) m_last_mask=GLTM->get("graphics/objects/ship-gravis-1.png");

	if (m_thrusting) {
		if (((m_cycle/4)%2)==0) m_last_tile=GLTM->get("graphics/objects/ship-gravis-2.png");
						   else m_last_tile=GLTM->get("graphics/objects/ship-gravis-3.png");
	} else {
		m_last_tile=GLTM->get("graphics/objects/ship-gravis-1.png");
	} // if 
