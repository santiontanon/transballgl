...
bool TGLobject_ship_accura::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (k->m_button[0] && !k->m_old_button[0] && m_fuel>40) {
		...
		if (m_missile_side==0) {
			bullet=new TGLobject_bullet_hmissile(float(get_x()+(cos_degree(a)*8)+(sin_degree(a)*7)),float(get_y()+(sin_degree(a)*8)-(cos_degree(a)*7)),0,m_angle,4,1,GLTM->get("graphics/objects/bullet-small-grey-missile1.png"),GLTM->get("graphics/objects/bullet-small-grey-missile2.png"),this);
			m_missile_side=1;
		} else {
			bullet=new TGLobject_bullet_hmissile(float(get_x()+(cos_degree(a)*8)-(sin_degree(a)*7)),float(get_y()+(sin_degree(a)*8)+(cos_degree(a)*7)),0,m_angle,4,1,GLTM->get("graphics/objects/bullet-small-grey-missile1.png"),GLTM->get("graphics/objects/bullet-small-grey-missile2.png"),this);
			m_missile_side=0;
...
void TGLobject_ship_accura::draw(glm::mat4 transform, class GLTManager *GLTM)
{
	if (m_last_mask==0) m_last_mask=GLTM->get("graphics/objects/ship-accura-1.png");

	if (m_thrusting) {
		if (((m_cycle/4)%2)==0) m_last_tile=GLTM->get("graphics/objects/ship-accura-2.png");
						   else m_last_tile=GLTM->get("graphics/objects/ship-accura-3.png");
	} else {
		m_last_tile=GLTM->get("graphics/objects/ship-accura-1.png");
	} // if 
