...
bool TGLobject_fastcannon_down::cycle(VirtualController *k,class TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	case 1: ship=map->object_exists("TGLobject_ship",get_x()-16,get_y()-240,get_x()+48,get_y()+32);
			if (ship!=0) {
				// fire
				map->add_object_back(new TGLobject_bullet(get_x()+15,get_y(),0,0,5,1,GLTM->get("graphics/objects/bullet-grey.png"),this));
				m_state=64;
...
void TGLobject_fastcannon_down::draw(glm::mat4 transform, class GLTManager *GLTM)
{
	m_last_tile=GLTM->get("graphics/objects/fastcannon-techno-down1.png");
    if (m_last_tile!=0) {
