...
void TGLobject_laser_right::draw(glm::mat4 transform, class GLTManager *GLTM)
{
	int local_cycle=((m_animation_offset+m_cycle)/96)%2;
	if (local_cycle==0) m_last_tile=GLTM->get("graphics/objects/laser-right1.png");
				   else m_last_tile=GLTM->get("graphics/objects/laser-right2.png");
    if (m_last_tile!=0) {
