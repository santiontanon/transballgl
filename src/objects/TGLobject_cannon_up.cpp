...
bool TGLobject_cannon_up::cycle(VirtualController *k,class TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	case 1: ship=map->object_exists("TGLobject_ship",get_x()-16,get_y(),get_x()+48,get_y()+240);
			if (ship!=0) {
				// fire
				map->add_object_back(new TGLobject_bullet(get_x()+17,get_y()+30,0,180,2,1,GLTM->get("graphics/objects/bullet-grey.png"),this));
				m_state=128;
...
void TGLobject_cannon_up::draw(glm::mat4 transform, class GLTManager *GLTM)
{
	switch(m_type) {
	case 0:
		if (m_state==0) m_last_tile=GLTM->get("graphics/objects/cannon-rock-up1.png");
		if (m_state==1) m_last_tile=GLTM->get("graphics/objects/cannon-rock-up2.png");
		if (m_state>126) m_last_tile=GLTM->get("graphics/objects/cannon-rock-up3.png");
		if (m_state>124 && m_state<=126) m_last_tile=GLTM->get("graphics/objects/cannon-rock-up4.png");
		if (m_state>122 && m_state<=124) m_last_tile=GLTM->get("graphics/objects/cannon-rock-up5.png");
		if (m_state>118 && m_state<=122) m_last_tile=GLTM->get("graphics/objects/cannon-rock-up4.png");
		if (m_state>114 && m_state<=118) m_last_tile=GLTM->get("graphics/objects/cannon-rock-up3.png");
		if (m_state>1 && m_state<=114) m_last_tile=GLTM->get("graphics/objects/cannon-rock-up2.png");
		break;
	case 1:
		if (m_state==0) m_last_tile=GLTM->get("graphics/objects/cannon-techno-up1.png");
		if (m_state==1) m_last_tile=GLTM->get("graphics/objects/cannon-techno-up2.png");
		if (m_state>126) m_last_tile=GLTM->get("graphics/objects/cannon-techno-up3.png");
		if (m_state>124 && m_state<=126) m_last_tile=GLTM->get("graphics/objects/cannon-techno-up4.png");
		if (m_state>122 && m_state<=124) m_last_tile=GLTM->get("graphics/objects/cannon-techno-up5.png");
		if (m_state>118 && m_state<=122) m_last_tile=GLTM->get("graphics/objects/cannon-techno-up4.png");
		if (m_state>114 && m_state<=118) m_last_tile=GLTM->get("graphics/objects/cannon-techno-up3.png");
		if (m_state>1 && m_state<=114) m_last_tile=GLTM->get("graphics/objects/cannon-techno-up2.png");
		break;
	} // switch 
    if (m_last_tile!=0) {
