...
bool TGLobject_ship_xterminator::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (k->m_button[0] && !k->m_old_button[0] && m_fuel>96) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		bullet=new TGLobject_bullet_missile(float(get_x()+(cos_degree(a)*8)),float(get_y()+(sin_degree(a)*8)),0,m_angle,4,4,GLTM->get("graphics/objects/bullet-grey-missile1.png"),GLTM->get("graphics/objects/bullet-grey-missile2.png"),this);
...
void TGLobject_ship_xterminator::draw(glm::mat4 transform, class GLTManager *GLTM)
{
	if (m_last_mask==0) m_last_mask=GLTM->get("graphics/objects/ship-xterminator-1.png");

	if (m_thrusting) {
		if (((m_cycle/4)%2)==0) m_last_tile=GLTM->get("graphics/objects/ship-xterminator-2.png");
						   else m_last_tile=GLTM->get("graphics/objects/ship-xterminator-3.png");
	} else {
		m_last_tile=GLTM->get("graphics/objects/ship-xterminator-1.png");
	} // if 
