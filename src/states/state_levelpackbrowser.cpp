	...
	if (m_state_cycle==0) {
		...
		// Load the lp filenames:
		{
			...
			for(i=0;i<m_lpb_lp_names.size();i++) {
				...
				tmp=new char[strlen(getAt<char *>(m_lpb_lp_names,i))+9];
				sprintf(tmp,TGL_PLAY_MAPS_FOLDER "/%s",getAt<char *>(m_lpb_lp_names,i));
				...
				// Select level pack:			
				{
					...
					if (fp!=0) {
						...
						if (lp!=0) {
							...
							get_player_profile()->set_last_levelpack(getAt<char *>(m_lpb_lp_fullnames,m_lpb_lp_selected));
						} // if 
...
//	char buffer[255];
	...
	if (m_lpb_mouse_over_lp!=-1) {
        ...
        drawQuad(15,float(40+m_lpb_mouse_over_lp*22),SCREEN_X-80,20, 0.5f,1,0.5f,f);
	} // if
